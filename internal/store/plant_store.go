...
func (s *PlantStore) UpdateSlabskid(unitID string, slabskid model.SlabskidReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateBurner(unitID string, comb model.BurnerReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateSlabzone(unitID string, slabzone model.SlabzoneReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
