...
type SlabskidSwellWindow struct {
	clk    ProcessClock
	window Window
}

func NewSlabskidSwellWindow(clk ProcessClock) *SlabskidSwellWindow {
	return &SlabskidSwellWindow{clk: clk, window: NewWindow(model.SlabskidSwellSettleWindow)}
}

func (d *SlabskidSwellWindow) Settled(swellAt time.Time) bool {
	return d.window.Satisfied(d.clk, swellAt)
}

func (d *SlabskidSwellWindow) RequireSettled(swellAt time.Time) error {
