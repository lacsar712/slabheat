...
// ReheatfnDrivePulse counts reheatfn-side effects from FSM after hooks (acceptance).
var ReheatfnDrivePulse func()

func RegisterReheatfnDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if ReheatfnDrivePulse != nil {
			ReheatfnDrivePulse()
		}
		return nil
	})
