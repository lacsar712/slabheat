...
func (s *Server) Routes() http.Handler {
	...
	mux.HandleFunc("GET /slabskid/level", withTimeout(s.handleSlabskidLevel))
...
func (s *Server) handleWarmup(w http.ResponseWriter, r *http.Request) {
	...
	writeOK(w, map[string]interface{}{
		...
		"warmup": s.app.BurnerWarmupRemaining(),
	})
}
...
func (s *Server) handleSlabskidLevel(w http.ResponseWriter, r *http.Request) {
	if err := s.app.CheckSlabskidLevel(s.app.Snapshot()); err != nil {
		if code, ok := classifySlabskidError(err); ok {
